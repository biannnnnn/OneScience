#!/usr/bin/env python3
"""Generate leakage-safe venue-score features for decision cases.

Mirrors run_reviews.py but posts one manuscript at a time to /v1/venue-scores.
The decision_label is never sent to the scorer; only the neutral target venue
name, title and manuscript text are exposed.
"""
...
def validate_target_venues(cases):
    for case in cases:
        ...
        if matched:
            raise SystemExit(
                "target_venue 包含疑似结果字段 {!r}（case_id={}），拒绝生成评分特征。".format(
                    matched, case.get("case_id")
                )
            )


def manuscript_text(case, max_characters=15000):
    paragraphs = (case.get("manuscript") or {}).get("paragraphs") or []
    text = "\n\n".join(item.get("text") or "" for item in paragraphs)
    return text.strip()[:max_characters]


def call_venue_scorer(case, base_url, api_key, timeout, busy_retries, max_characters):
    # decision_label is deliberately not copied into this request.
    manuscript = case.get("manuscript") or {}
    payload = {
        "request_id": case["case_id"],
        "review_language": "en" if manuscript.get("language") == "en" else "zh-CN",
        ...
        "papers": [
            {
                ...
                "language": manuscript.get("language") or "en",
                "text": manuscript_text(case, max_characters=max_characters),
            }
        ],
    }
    ...
    request = urllib.request.Request(
        base_url.rstrip("/") + "/v1/venue-scores",
    ...
    for attempt in range(busy_retries + 1):
        try:
            ...
            batch = result.get("score_batch") or {}
            scores = batch.get("scores") or []
            if not scores:
                return {
                    "case_id": case["case_id"],
                    "status": "error",
                    "error": "EMPTY_SCORE_BATCH",
                    "score": None,
                    "model_trace": None,
                }
            return {
                ...
                "status": "ok",
                "score": scores[0],
                "model_trace": batch.get("model_trace"),
...
def main():
    parser = argparse.ArgumentParser(description="Run frozen venue scorer over decision cases")
    ...
    parser.add_argument("--max-characters", type=int, default=15000)
