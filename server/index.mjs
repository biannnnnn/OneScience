...
async function createProjectFromDocument(document, profile) {
...
app.get('/api/health', (_request, response) => {
  response.json({ ok: true, service: 'onescience-submission-agent', version: '0.1.0' });
});
...
app.post('/api/analyze', upload.single('paper'), async (request, response, next) => {
  try {
    ...
    const project = await createProjectFromDocument(extracted, {
...
app.post('/api/projects/:id/recommend', async (request, response, next) => {
  try {
    const project = await getProject(request.params.id);
    if (!project) return response.status(404).json({ error: '未找到该投稿项目。' });
    const recommendations = recommendJournals(project, request.body);
    response.json(await patchProject(project.id, { recommendations, stage: Math.max(project.stage, 2), status: '待选择期刊' }));
  } catch (error) {
    next(error);
  }
});
