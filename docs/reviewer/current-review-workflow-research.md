...
- 基座：当前仓库的 Qwen 7B/8B Reviewer + LoRA；
- 输入：初始投稿版本、目标期刊范围、该刊证据偏好；
- 输出：原创性、方法可靠性、证据充分性、清晰度、可复现性、期刊适配度，以及总体准备度；
- 标签：OpenReview/PeerRead 中可公开使用的数值评分与结构化意见，先统一量纲，再做 ordinal regression 或多任务分类；
- 训练：domain SFT → schema SFT → 冻结版本；
- 评估：维度分 MAE/Spearman、总体等级 Macro-F1、证据引用有效率、跨年份与跨 venue 稳定性。
...
当冻结小模型在线时，每篇论文得到 0–100 的期刊条件准备度分，同时保留 verdict、置信度、正向因素、风险和模型签名。用户稿件在每本候选期刊下单独评分，不能把一份通用分复制到所有期刊。
